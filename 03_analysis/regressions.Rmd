...
```{r, echo = F, warning = F, message = F, eval = F, profile = F}
...
### FOREST PLOT - UGLY FIRST TRY

## need to create different shape for glm vs glmer and have it be on different level on y axis
...
forest_plotDF <- forest_plotDF %>% select(-c(4:6))

# plot
ggplot(data = forest_plotDF, aes(group = "Variable")) +
  geom_vline(xintercept = 1, linetype = "dashed") +
  geom_point(aes(x = Estimate, y = as.factor(Variable)), size = 3) +
  geom_segment(aes(x = Lower_95, xend = Upper_95, y = as.factor(Variable), yend = as.factor(Variable)), 
               size = 1.5) +
  xlab("Estimate") + 
  #xlim(0.8, 1.2) +
  ylab("") +
