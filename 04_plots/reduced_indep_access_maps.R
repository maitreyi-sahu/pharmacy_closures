...
# Load data
fig1_data <- read.csv(paste0(data_dir, "reduced_indep_access_by_state_for_mapping.csv")) %>% 
  rename(state = state_name)
fig2_data <- read.csv(paste0(data_dir, "reduced_indep_access_counties_for_mapping.csv")) %>% 
...
pdf(paste0(out_dir, "fig1_states_reduced_access_indep.pdf"), height = 15, width = 25)

formatPlot(

  plot_usmap(data = fig1_data, regions = "states", values = "pct_counties_rdcd_indep_access") + 
    geom_polygon(data = usmapdata::us_map(regions = "states"),
                 aes(x, y, group = group), fill = NA, size = .25, color = "black") +
    labs(title = "Percent of Counties with Reduced Access to Independent Pharmacies, by State, 2021",
         subtitle = paste0("")) + 
...
pdf(paste0(out_dir, "fig2_counties_reduced_access_indep.pdf"), height = 15, width = 25)

formatPlot(

  plot_usmap(data = fig2_data, regions = "counties", values = "reduced_indep_access_county", color = NA) + 
    ...
    labs(title = "Counties with Reduced Access to Independent Pharmacies in 2021",
         subtitle = paste0("")) + 
