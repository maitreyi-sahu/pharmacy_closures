...
```{r, echo = F}
df <- data %>% mutate(`Open 24 Hours?` = ifelse(open24hours == "Y", "Yes", "No"))

ggplot(data = df, aes(fill = `Open 24 Hours?`, x = `Year of Closure`, y = closure)) + 
  geom_bar(position="stack", stat="identity") +
  ylab("Frequency") +
  theme_bw()  
...
plot_usmap(data = stateDF, regions = "states", values = "closures_n") + 
  ...
  scale_fill_continuous(name = "Closures (n)", label = scales::comma) + 
